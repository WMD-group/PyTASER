...
class TASPlotter:
    ...
    Args:
        container: TAS container class as generated by tas.
            tas_tot: overall TAS spectrum for a material under the specified conditions
            tas_decomp: TAS spectrum across the energy mesh for a specific band transition i (initial) -> f (final) [dict]
            energy_mesh: Energy mesh of spectra in eV, with an interval of 'step'.
            jdos_light_tot: overall JDOS (pump-on) for a material under the specified conditions
            jdos_light_decomp: JDOS (pump-on) across the energy mesh for a specific band transition i (initial) -> f (final) [dict]
            jdos_dark_tot: overall JDOS (pump-off) for a material under the specified conditions
        ...
        temp: Temperature of TAS spectrum. Will affect occupancy according to FD distrib. (K)
        conc: Number of charge carriers (#electrons = #holes, assumes no recombinations)
    """
    ...
    def get_plot(self, relevant_transitions=None, xaxis="wavelength", xmin=None, xmax=None, ymin=None, ymax=None,
                 yaxis="TAS (deltaT)"):
        """
        Args:
            relevant_transitions: List containing individual transitions to be displayed in the plot alongside the total plot.
                                  If material is not spin-polarised, only write the bands involved [(1,6),(2,7),(8,9)...]
                                  If spin-polarised, include the type of spin involved in transition[(1,6, 'spin = down'),(2,7, 'spin = down'),(8,9, 'spin = up')...]
    ...
    ######### deltaT/T IS CURRENTLY NON-FUNCTIONING - we encounter a 'divide by `zero error' ###########
        ...
        #             plt.plot(energy_mesh, self.denom_tas_decomp[transition], label=transition)
            ...
            plt.plot(energy_mesh, self.jdos_light_tot, label='JDOS (light)', color='black', lw=1.5)
            plt.plot(energy_mesh, self.jdos_dark_tot, label='JDOS (dark)', color='blue', lw=1.5)
        ...
        plt.title(abs_label + ' spectrum of ' + self.material_name + ' at T = ' + str(self.temp) + ' K, n = ' + str(self.conc))
