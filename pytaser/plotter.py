...
class TASPlotter:
    ...
    def fill_container_das(self, container):
        ...
        if self.bandgap_ref <= 0.01:
            print(
                "Band gap < 0.01. Settiing to 0.01 eV to avoid division by zero when converting energies from eV to "
                "wavelength in nm. "
            )
        ...
        **kwargs,
        ...
        Args:
            ...
            yaxis: What spectral data to plot. If yaxis = "tas" (default), will plot the
                ...
                If yaxis = "tas_absorption_only", will instead plot the predicted TAS spectrum considering only the
                change in absorption, with stimulated emission neglected. If yaxis = "jdos", will plot the joint
                density of states before & after illumination. If yaxis = "alpha", will plot the effective absorption
                coefficient (a, including absorption and stimulated emission) before and after illumination. If yaxis
                = "das", will plot the difference between the optical absorption spectra of the new system and the
                reference system, together with the absorption spectra If yaxis = "jdos_das", will plot the
                difference between the JDOS of the new system and the reference system, together with the JDOS of
                both systems. invert_axis: If true, x-axis decreases from left-right. If false (default), x-axis will
                increase from left-right. Especially relevant when switching between wavelength and electronvolts.
                **kwargs: Additional arguments to be passed to matplotlib.pyplot.legend(); such as `ncols` (number of
                columns in the legend), `loc` (location of the legend), `fontsize` etc. (see
        ...
        def _rescale_overlapping_curves(list_of_curves):
            ...
            for curve in list_of_curves:  # Find the local maxima of each curve
                if curve is not None and np.max(np.abs(curve)) / max_curve > 0.05:
                    local_extrema_indices = argrelextrema(np.abs(curve), np.greater)[0]
                    relative_local_extrema = [
                        (idx, round(np.abs(curve)[idx] / max_curve, 2)) for idx in local_extrema_indices
                    ]
                    ...
                    while any(
                        ...
                        local_extrema_indices = argrelextrema(np.abs(curve), np.greater)[0]
                        relative_local_extrema = [
                            (i, round(np.abs(curve)[i] / max_curve, 2)) for i in local_extrema_indices
                        ]
