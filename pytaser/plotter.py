...
class TASPlotter:
    ...
    def get_plot(
        ...
        **kwargs,
        ...
        as generated by ``TASGenerator().generate_tas()`` or in the DAS
        container class generated by ``DASGenerator().generate_das()``. If the
        ``TASGenerator`` was not generated from VASP outputs, then the output
        TAS is generated using the change in joint density of states (JDOS)
        under illumination, with no consideration of oscillator strengths
        ("Total TAS (ΔJDOS only)") - this behaviour can also be explicitly
        chosen by setting ``"yaxis"`` to ``"jdos_diff"``.
            ...
            if self.alpha_light_dict is not None and "jdos_diff" not in yaxis.lower():
                ...
                if "tas" in yaxis.lower():
                    ...
                    if relevant_transitions == "auto":
                        list_of_transitions = cutoff_transitions(
