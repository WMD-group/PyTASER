...
class dasplotter:
    """
    Class to generate a matplotlib plot of the DAS or JDOS spectra, with a specific energy
    mesh, material, and conditions.

    Args:
        container: DAS container class as generated by DASGenerator().generate_das().
    ...
    def get_plot(
        ...
        **kwargs,
        ...
        """
        Plots DAS spectra using the data generated in the DAS container class as generated by
        DASGenerator().generate_das(). If the DASGenerator was not generated from VASP outputs,
        then the output DAS is generated using the change in joint density of states (JDOS) under
        illumination, with no consideration of oscillator strengths ("Total DAS (ΔJDOS only)") –
        this behaviour can also be explicitly chosen by setting "yaxis" to "jdos_diff".

        Otherwise, the output DAS is generated considering all contributions to the predicted TAS
        ...
        if yaxis.lower() not in [
            ...
            raise ValueError(
                f"Invalid yaxis '{yaxis}' specified, must be one of: 'das', "
                f"'jdos'!"
            )
        ...
        if bg_ref is not None and bg_newSys is not None:
            ...
            plt.plot(x_bg_ref, y_bg, label="Reference Bandgap",color="red", ls="--")
            plt.plot(x_bg_newSys, y_bg, label="New System Bandgap",color="blue", ls="--")
